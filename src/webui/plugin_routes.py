    ...
    for dotted_key, v in dotted_items:
        ...
        for idx, part in enumerate(parts):
            ...
            if not is_last:
                ...
                if part in current and isinstance(current[part], dict) and isinstance(value, dict):
...
def coerce_types(schema_part: Dict[str, Any], config_part: Dict[str, Any]) -> None:
    ...
    for key, schema_val in schema_part.items():
        ...
        if isinstance(schema_val, ConfigField):
            if schema_val.type is list and isinstance(value, str):
    ...
    try:
        ...
        plugin_instance = next(
            (
                instance
                for loaded_plugin_name in plugin_manager.list_loaded_plugins()
                if (instance := plugin_manager.get_plugin_instance(loaded_plugin_name))
                and (instance.plugin_name == plugin_id or instance.get_manifest_info("id", "") == plugin_id)
            ),
            None,
        )
