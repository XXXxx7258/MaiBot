    ...
    for dotted_key, v in dotted_items:
        ...
        for idx, part in enumerate(parts):
            ...
            if not is_last:
                ...
                # 最后一级：如已有且为 dict 且 v 是 dict，可合并，否则覆盖
                if part in current and isinstance(current[part], dict) and isinstance(v, dict):
                    current[part].update(v)
                else:
                    current[part] = v
...
def coerce_types(schema_part: Dict[str, Any], config_part: Dict[str, Any]) -> None:
    ...
    for key, schema_val in schema_part.items():
        if (
            key in config_part
            and isinstance(schema_val, ConfigField)
            and schema_val.type is list
            and isinstance(config_part[key], str)
        ):
            config_part[key] = [item.strip() for item in config_part[key].split(",") if item.strip()]
        elif isinstance(schema_val, dict) and key in config_part and isinstance(config_part[key], dict):
